...
cur.execute("SELECT x, y, z, timestamp, date, time FROM loggerdata WHERE rowid > ? AND rowid < ?", (0, 200))
data = cur.fetchall()
...
ax.plot(timestampnorm, xdata, label='x acceleration', linewidth=1, color = "#000000")
ax.plot(timestampnorm, ydata, label='y acceleration', linewidth=1, color = "#555555")
ax.plot(timestampnorm, zdata, label='z acceleration', linewidth=1, color = "#aaaaaa")
